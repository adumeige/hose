...
## 017 — Postgres feed: payload-in-NOTIFY with bounded fallback
The notify trigger embeds the row's payload in the notification when the message stays under ~7.5KB (pg_notify's 8KB ceiling); oversized rows notify key+version only and the poller selects back, dropping the event if the row's version has already moved on — correct because a newer notification is necessarily queued behind it, and the core's version guard conflates anyway. First `changeFeed` blocks until LISTEN is active, otherwise writes racing the poller's startup notify into the void (found by the per-key-ordering contract test). Databases (not schemas) isolate test stores: NOTIFY scopes per database.

## 018 — Postgres triad mapping
One table per entity type (`key TEXT PK, version TEXT, payload JSONB, payload_class TEXT`), created lazily with its trigger on first touch. Relations resolve through `follow` (batched get); a dedicated edge/link table joins the schema the day the SPI grows an edge concept — same position as DECISIONS #004.
